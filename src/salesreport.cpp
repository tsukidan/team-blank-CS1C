    ...
    ui(new Ui::Salesreport)
{
    ...
    query.prepare("SELECT items.id, items.name, (items.price/100.0), purchases.quantity, (purchases.quantity * items.price / 100.0) AS revenue FROM items INNER JOIN purchases ON purchases.item_id=items.id WHERE purchases.date=?");
...
void Salesreport::updateMembers(){
    ...
    QSqlQuery query;
    query.prepare("SELECT DISTINCT members.name, members.executive FROM members INNER JOIN purchases ON purchases.member_id=members.id WHERE purchases.date=?");
    query.addBindValue((ui-> dateEdit -> date()).toString("yyyy-MM-dd"));
    if (!query.exec()) {
      qDebug() << "Query Fail member list"
               << query.lastError().text();
      return;
    }
    int numStandard = 0;
    ...
    ui -> numExec -> setText(QString("%1 Executive Members Shoped").arg(numExecutive));
}
...
void Salesreport::on_memberType_currentIndexChanged(int index)
{
    ...
    switch(index){
    case 1:
        query.prepare("SELECT items.id, items.name, (items.price/100.0), purchases.quantity, (purchases.quantity * items.price / 100.0) AS revenue FROM items INNER JOIN members ON purchases.member_id=members.id INNER JOIN purchases ON purchases.item_id=items.id WHERE purchases.date=? AND members.executive=false");
        break;
    case 0:
        query.prepare("SELECT items.id, items.name, (items.price/100.0), purchases.quantity, (purchases.quantity * items.price / 100.0) AS revenue FROM items INNER JOIN members ON purchases.member_id=members.id INNER JOIN purchases ON purchases.item_id=items.id WHERE purchases.date=? AND members.executive=true");
        break;
    default:
        break;
    }
